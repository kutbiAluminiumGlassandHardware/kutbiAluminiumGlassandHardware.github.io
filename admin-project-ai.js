...
async function dispatch(id,t){var r=await fetch('https://api.github.com/repos/'+OWNER+'/'+REPO+'/dispatches',{method:'POST',headers:auth(t),body:JSON.stringify({event_type:'kutbi_ai_project_request',client_payload:{request_id:id}})});if(!r.ok){var d=await r.json().catch(function(){return{}});throw Error(d.message||('GitHub dispatch error '+r.status));}}
...
function init(){
 ...
 if($('kutbiProjectPrompt')){
  if($('kutbiGenerateProject')&&!$('kutbiGenerateProject').dataset.kutbiBound){$('kutbiGenerateProject').dataset.kutbiBound='1';$('kutbiGenerateProject').addEventListener('click',generate);}
  return;
 }
