const OWNER='kutbiAluminiumGlassandHardware',REPO='kutbiAluminiumGlassandHardware.github.io',BRANCH='main',API=`https://api.github.com/repos/${OWNER}/${REPO}/contents/`,RAW=`https://raw.githubusercontent.com/${OWNER}/${REPO}/${BRANCH}/`,DIR='images/projects',MAX=2*1024*1024,MAX_FILES=150,$=id=>document.getElementById(id);
...
function decode(s){const raw=String(s||'').replace(/\n/g,'');if(!raw)return '';try{return decodeURIComponent(escape(atob(raw)))}catch(e){return atob(raw)}}function encode(s){return btoa(unescape(encodeURIComponent(s)))}
